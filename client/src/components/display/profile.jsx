    ...
    useEffect(() => {
        const getCollection = async() =>{
          try {
            ...
            const filter = data.docs.filter((user) => user.id == auth.currentUser.uid).map(async(doc) => {
  ...
  return (
    ...
    <div>
        ...
        <div className='grid grid-cols-3 gap-y-10'>
            <h1 className="pt-32 text-white col-start-2 text-center font-bold text-xl">Profile</h1>
        </div>
        {
            OperatorCollection ? 
            <div>
